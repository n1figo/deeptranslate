...
def translate_text_in_chunks(text, target_language="Korean", chunk_size=500, delay=6):
    text_splitter = CharacterTextSplitter(
        separator="\n",
    ...
    for chunk in chunks:
        response = trans_chain({'trans': chunk})
        translated_chunks.append(response['translate'])
        time.sleep(delay)  # Adding delay to respect rate limit
...
if uploaded_file is not None:
    ...
    if text:
        ...
        translated_text = translate_text_in_chunks(text, target_language)
